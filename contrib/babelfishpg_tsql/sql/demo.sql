create or replace view sys.demo_view as
select distinct
...
where c.contype = 'f';
GRANT SELECT ON sys.demo_view TO PUBLIC;

CREATE FUNCTION sys.demo_f1() RETURNS sys.datetimeoffset
    ...
    LANGUAGE SQL;
GRANT EXECUTE ON FUNCTION sys.demo_f1() TO PUBLIC;

CREATE FUNCTION sys.demo_f2(text,text) RETURNS text AS $$
  SELECT COALESCE($1,$2);
$$
LANGUAGE SQL;
GRANT EXECUTE ON FUNCTION sys.demo_f2(text,text) TO PUBLIC;
